...
class A_UpdateHandler(tornado.web.RequestHandler):
	...
	def post(self):
		...
		if not self.request.connection.stream.closed():
			self.write({"messages_a":g_a_messages})
...
class B_UpdateHandler(tornado.web.RequestHandler):
	...
	def post(self):
		...
		if num_seen == len(g_b_messages):
			self._future = tornado.concurrent.Future()
			g_b_waiters.add(self._future)
			yield self._future
		if not self.request.connection.stream.closed():
			self.write({"messages_b":g_b_messages})
	def on_connection_close(self):
...
class UpdateCompareHandler(tornado.web.RequestHandler):
	@tornado.gen.coroutine
	def post(self):
		correct = int(self.get_argument("correct",0))
		if correct == g_correct:
			print "in correct = g_correct"
			self._future = tornado.concurrent.Future()
	...
	def main():
		app = tornado.web.Application(
			[ (r"/",	   MainHandler),
			  ...
			  (r"/update_compare",	  UpdateCompareHandler)
			   ],
